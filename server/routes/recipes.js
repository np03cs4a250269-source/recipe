...
router.post("/recipes", verifyToken, upload.single("image"), (req, res) => {
  const { name, category, difficulty, description, instructions, ingredients } =
    req.body;
  ...
  db.query(
    ...
    (err, result) => {
      ...
      if (parsedIngredients.length === 0) {
        return res.status(201).json({ message: "Recipe added", id: recipeId });
      }
